...
def test_get(redis, token0):
    ...
    redis.set(k, token0)
    assert config.get(k, redis=redis) == token0


def test_get_with_default(redis, token0, token1):
    k = make_key("test-get-with-default-0")
    redis.set(k, token0)
    assert config.get(k, default=token1, redis=redis) == token0


def test_get_unset(redis):
    k = make_key("test-get-unset-0")
    assert config.get(k, redis=redis) is None
...
def test_update(redis, token0, token1):
    ...
    redis.set(k, token0)
    assert config.get(k, redis=redis) == token0
    config.update(k, token1, redis=redis)
    assert config.get(k, redis=redis) == token1
...
def test_all_keys(redis, token0):
    gen = config.all_keys(redis=redis)
    ...
    realised = list(config.all_keys(redis=redis))
