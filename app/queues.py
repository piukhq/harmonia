...
class StrictQueue:
    def __init__(
        self, transport_dsn: str, *, name: str, schema_class, retry: bool = True
    ) -> None:
    ...
    def _retry_callback(self, exc: Exception, interval: float) -> None:
        self.log.warning(
            f"Failed to connect to RabbitMQ @ {self.connection.hostname}: {exc}. Retrying in {interval}s…"
        )

    def _get_headers(self, override_headers: t.Optional[dict]) -> dict:
        headers = {"X-Queued-At": pendulum.now().int_timestamp, "X-Retry-Count": 0}
    ...
    def push(
        self,
        obj: t.Union[dict, list],
        *,
        many: bool = False,
        override_headers: dict = None,
    ) -> None:
        self.log.debug(
            f"Pushing {type(obj).__name__} to queue '{self.queue.name}', many: {many}."
        )
        ...
        * a dictionary of message headers (e.g. retry count, queue time, et cetera)
        ...
        class Worker(ConsumerMixin):
            ...
            def get_consumers(self, consumer_class: t.Type, channel) -> list:
                return [
                    consumer_class(
                        queues=[self.queue], callbacks=[self.message_callback]
                    )
                ]

            def on_connection_error(self, exc: Exception, interval: float) -> None:
                self.parent.log.warning(
                    f"Lost connection to broker ({exc}), will retry in {interval}s."
                )
        ...
        def receive_message(body: dict, message: Message) -> None:
            ...
            try:
                ...
                if raise_exceptions is True:
                    ...
                    self.log.error(
                        f"Message handler '{message_callback.__name__}' on the '{self.queue.name}' queue "
                ...
                if acknowledge is not False:
                    ...
                    self.log.debug(
                        f"Message handler '{message_callback.__name__}' on the '{self.queue.name}' queue "
                        "returned false. Message has not been acknowledged."
                    )

        worker = Worker(
            self.connection,
            self.queue,
            message_callback=receive_message,
            parent=self,
            once=once,
        )
        worker.run()
...
scheme_import_queue = StrictQueue(
    settings.AMQP_DSN,
    name="imports-scheme",
    schema_class=schemas.SchemeImportQueueSchema,
)
payment_import_queue = StrictQueue(
    settings.AMQP_DSN,
    name="imports-payment",
    schema_class=schemas.PaymentImportQueueSchema,
)
matching_queue = StrictQueue(
    settings.AMQP_DSN, name="matching", schema_class=schemas.MatchingQueueSchema
)
export_queue = StrictQueue(
    settings.AMQP_DSN, name="export", schema_class=schemas.ExportQueueSchema
)
pending_export_queue = StrictQueue(
    settings.AMQP_DSN,
    name="export-pending",
    schema_class=schemas.PendingExportQueueSchema,
)
