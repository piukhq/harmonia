...
class AmexAuth(QueueAgent):
    ...
    @cache
    def should_import(self, merchant_slug: str) -> bool:
        # Spotting and streaming merchants cannot use Amex auth transactions.
        # Use the registered agents from streaming and matching (spotting) to check the merchant
        # If the merchant is a spotting or streaming agent, then do not import, return false for this method
        # Streaming agents are checked first since this is less overhead then checking matching agents
    ...
    def to_transaction_fields(self, data: dict) -> list[PaymentTransactionFields]:
        ...
        merchant_slugs = self.get_merchant_slugs(data)
        return [
            ...
            for merchant_slug in merchant_slugs
            if self.should_import(merchant_slug)
        ]
