...
class MatchedTransaction(Base, ModelMixin):
    ...
    user_identity = s.orm.relationship(
        "UserIdentity", uselist=False, back_populates="matched_transaction"
    )
...
class UserIdentity(Base, ModelMixin):
    ...
    matched_transaction = s.orm.relationship(
        "MatchedTransaction", uselist=False, back_populates="user_identity"
    )
