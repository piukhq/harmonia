...
class CooperativeProducer(object):
    ...
    def user_request_to_hermes(self):
        register_resp = requests.post(
            f"{HERMES_URL}/users/register",
            json={"email": f"{uuid4()}@txmatch.com", "password": "Password01"},
        )
        register_resp.raise_for_status()
        user_register_headers = {
            "Authorization": f"Token {register_resp.json()['api_key']}"
        }
    ...
    def payment_card_request_to_hermes(self, user_register_headers, transaction):
        ...
        try:
            ...
            pca_resp = requests.post(
                f"{HERMES_URL}/payment_cards/accounts",
                json=payment_card,
                headers=user_register_headers,
            )
            pca_resp.raise_for_status()
            self.log.debug(
                f"Payment card in Hermes has been registered {pca_resp.json()}"
            )
        except Exception as error:
            self.errors_in_processes.append(error)
        return payment_card
    ...
    def scheme_account_request_to_hermes(self, user_register_headers):
        ...
        self.log.debug(
            f"Scheme account in Hermes has been registered {sa2_resp.json()}"
        )
        return scheme_card, sa2_resp

    def credentials_request_to_hermes(
        self, sa2_resp, user_register_headers, scheme_card_number
    ):
        credentials_json = {
            "password": "1234567",
            "email": "email=bv@gmail.com",
            "card_number": scheme_card_number,
        }
        ...
        status_resp = requests.post(
            f"{HERMES_URL}/schemes/accounts/{sa2_resp.json()['id']}/status",
            json=json_status,
            headers=headers,
        )
    ...
    def all_processes(self):
        ...
        for transaction in file["transactions"]:
            ...
            payment_card = self.payment_card_request_to_hermes(
                user_register_headers, transaction
            )
            scheme_card_number, sa2_resp = self.scheme_account_request_to_hermes(
                user_register_headers
            )
            self.credentials_request_to_hermes(
                sa2_resp, user_register_headers, scheme_card_number
            )
